...
class LinkedList {
	...
	insertTail(value) {
		...
		if (!tail) this.head = newTail;
		else tail.next = newTail;
	}

	insert(value) {
		if (!this.head) this.insertHead(value);
		else this.insertTail(value);
	}
	...
	insertAt(index, value) {
		...
		if (!nodeBeforeIndex) return this.insertTail(value);
	...
	removeAt(index) {
		...
		if (!nodeBeforeIndex) return this.removeTail();
